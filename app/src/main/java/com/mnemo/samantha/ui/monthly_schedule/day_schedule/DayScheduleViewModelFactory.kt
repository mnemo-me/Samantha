...
class DayScheduleViewModelFactory(val year: Int, val month: Int, val date: Int, val repository: Repository) : ViewModelProvider.Factory {
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(DayScheduleViewModel::class.java)){
            return DayScheduleViewModel(year, month, date, repository) as T
        }
